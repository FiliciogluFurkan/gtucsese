...
const CourtManagement = (): JSX.Element => {
  ...
  const handleSave = (fieldId: number) => {
    const updatedFields = fields.map((field) =>
      field.field.id === fieldId ? { ...field } : field
    );
  ...
  const handleChange = (
    ...
    fieldKey: keyof FootballCourtFields["field"],
    index: number
  ) => {
  ...
  const handleChange2 = (
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>,
    fieldKey: keyof typeof newField
  ) => {
      ...
      >
        ...
        <Box sx={{ textAlign: "center", padding: "2rem" }}>
          ...
          <Box
            ...
            onClick={handleOpen}
          >
            <Box sx={{ fontSize: "2rem", fontWeight: "bold", color: "#777" }}>
              +
            </Box>
