...
const CourtManagement = (): JSX.Element => {
  ...
  const SaveEditedCourt = (fieldId: string) => {
    const updatedFields = courts.map((court) =>
      court.id === fieldId ? { ...court } : court
    );
  ...
  const handleAddCourt = async () => {
    ...
    try {
      ...
      const courtData = {
        ...
        description: String(
          "bu halısaha 7+7 kişiliktir ve çimleri daha yeni yenilenmiştir"
        ),
      ...
      formData.append(
        "files",
        new File([fileBlob], "court-image.jpg", {
  ...
  const handleChange2 = (
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>,
    fieldKey: keyof typeof newCourt
  ) => {
                  ...
                  {editingFields[index] ? (
                    <Box sx={{ display: "flex", gap: "1rem" }}>
                      <TextField
                        type="number"
                        value={court.width}
                        onChange={(e) => handleChange(e, "width", index)}
                        size="small"
                        sx={{ width: "70px" }}
                      />
                      <TextField
                        type="number"
                        value={court.height}
                        onChange={(e) => handleChange(e, "height", index)}
                        size="small"
                        sx={{ width: "70px" }}
                      />
                    </Box>
            ...
            >
              {editingFields[index] ? (
                <Button
                  ...
                  onClick={() => SaveEditedCourt(court.id)}
                >
                  Kaydet
                </Button>
              ) : (
        ...
        <Box sx={{ textAlign: "center", padding: "2rem" }}>
          ...
          >
            ...
            {selectedImage ? (
              ...
              <>
                <Box
                  sx={{ fontSize: "2rem", fontWeight: "bold", color: "#777" }}
                >
                  +
                </Box>
